...
var getBlocks = function (dest, dir, content) {
  ...
  lines.forEach(function (l) {
    ...
    if (block && last) {
      ...
      if (asset && asset[2]) {
        //for less block - always skip script tag
        if(!(last.type === 'less' && scriptTag.test(l))) {
...
HTMLProcessor.prototype.replaceWith = function replaceWith(block) {
  ...
  var media;
  if (block.type === 'css') {
    media = block.media ? ' media="' + block.media + '"' : '';
  ...
  } else if (block.type === 'less') {
    //less result should be css
    media = block.media ? ' media="' + block.media + '"' : '';
    result = block.indent + '<link rel="stylesheet" href="' + dest + '"' + media + '>';
  } else {
    result = '';
